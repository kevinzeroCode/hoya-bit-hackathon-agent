    ...
    async def run(self, *, plan, request, deadline):
        ...
        record = RawSourceRecord(
            ...
            metadata={
                ...
                # Provenance only. Reliability and independence group are derived
                # from this by the processor, never stated by the producer.
                "original_publisher": "coindesk.com",
                "original_page_fetched": True,
            },
        )
        return SimpleNamespace(
...
async def test_invalid_research_draft_is_rejected_before_arbiter() -> None:
    ...
    # Rejected during extraction completion (it cites no fetched record), which
    # discloses more precisely than the generic merge-time contract note.
    assert any(
        "捨棄引用不存在來源紀錄" in note or "Evidence 契約" in note
        for note in outcome.degradation_notes
    )
