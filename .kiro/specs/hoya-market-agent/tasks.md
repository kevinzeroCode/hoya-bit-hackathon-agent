...
- [x] **16. G1 semantic grounding recheck for qualitative claims** (engine + confidence wiring done 2026-08-03; live pipeline call deliberately not wired — see below)
  ...
  - **Spec:** `docs/Gold-Plan.md` §G1
  - **Scope decision made while implementing — read before wiring this into a live run:** grepping the whole tree found `evidence.ledger.confidence_signals_for_claim` has **zero call sites outside its own tests** — the deterministic grounding-to-confidence link Task 5 built is itself not wired into the live pipeline yet (the live Arbiter path uses its own separate `apply_confidence_caps`). Wiring a *new* live LLM call (semantic grounding) ahead of its own deterministic prerequisite being live, with no way to verify either one against real Bedrock in this session (no live AWS credentials available), was judged the wrong order of operations. What shipped: the semantic-grounding engine itself (tested against a fake `LLMClient`, same rigor as every other reasoning-layer component) and the `confidence_signals_for_claim`/`_is_grounded` extension to consume its verdicts — both complete, tested, and **inert until something calls them**, so nothing about the current live pipeline's behavior changed.
  - **Files (as actually touched):**
    - Created: `src/hoya_agent/reasoning/semantic_grounding.py` — `semantic_ground(fact, content_reference, *, llm, deadline, max_tokens=300) -> SemanticVerdict`. Does not modify `evidence/grounding.py` (stays LLM-free per its own docstring) or any frozen `reasoning/` file.
    - Created: `prompts/semantic-grounding-v1.md`, registered as a new `"semantic_grounding"` stage in `reasoning/prompt_library.py::PROMPT_FILES` (one dict entry — the existing `SharedPromptContractTests` in `tests/unit/reasoning/test_prompt_library.py` iterate `PROMPT_FILES` dynamically and caught that the new prompt needed the same injection-resistance/no-advice/JSON-only clauses every other prompt states, which the first draft was missing).
    - Created: `tests/unit/reasoning/test_semantic_grounding.py` — 7 tests with a fake `LLMClient` (verified / contradicted / LLM failure → unverified / out-of-enum model output → unverified, never raises).
    - Modified: `src/hoya_agent/evidence/ledger.py` — `_is_grounded()`/`confidence_signals_for_claim()` gained an optional `semantic_status: dict[str, str] | None` lookup (evidence_id -> plain string, not the `reasoning/` enum — `ledger.py`'s own docstring says "no LLM" and importing a reasoning-layer type would create exactly the dependency that line means to prevent). A `GroundingStatus.partial` (numeric fabrication) verdict is never overridden by it; only `GroundingStatus.unverified` (no checkable atom at all) consults it. Omitting the parameter reproduces prior behavior exactly.
    - Modified: `tests/unit/evidence/test_grounding.py` — 1 new test proving a qualitative fact is rescued by a `"verified"` semantic status but a numerically-fabricated fact never is, even when its own semantic status also says `"verified"`.
    - **Not modified:** `src/hoya_agent/orchestration/pipeline.py` — see scope decision above.
  - [x] Write failing tests with a fake `LLMClient` proving: a purely qualitative fact whose `content_reference` plausibly supports it → `verified`; one that contradicts its source → `contradicted`; LLM failure/timeout → falls back to the existing deterministic `unverified`, never blocks the run.
  - [x] Implement the bounded call: one fact at a time, capped `max_tokens=300`, deadline passed through by the caller (whoever eventually wires it) rather than read from any global — this module owns no stage budget of its own.
  - [x] Feed the result into `confidence_signals_for_claim` — done as described above.
  - [x] Red lines: never mutates static `reliability`; adds no field to `EvidenceItem`/`EvidenceDraft`; any LLM error, timeout or out-of-enum output degrades to `unverified`, never raises.
  - [ ] **Not done:** calling `semantic_ground()` from `orchestration/pipeline.py` during a real run. See the scope decision above; this is the one remaining step for whoever picks this up, once `confidence_signals_for_claim` itself is wired into the live Arbiter path (a prerequisite this task discovered, not one it caused).
  - [x] Ran `python -m pytest tests/unit/reasoning/test_semantic_grounding.py tests/unit/reasoning/test_prompt_library.py tests/unit/evidence -q` (all passed) then the full non-live suite (1334 passed, up from 1326) and `ruff check .` (All checks passed).
  - **Acceptance:** **Engine and confidence-signal integration met; live-pipeline wiring explicitly deferred** (see scope decision). Nothing about the currently-shipped pipeline's behavior changed — both the deterministic and semantic grounding-to-confidence paths remain opt-in and unwired until a future task connects them.
  - **Commit:** `feat: add semantic recheck for qualitative fact-grounding`

- [x] **17. Implement H3 Conditional Debate** (engine done 2026-08-03; live pipeline wiring deliberately not done — see below)
  ...
  - **Design finding made while implementing, before writing any code:** `ConflictExtension`/`DisabledConflictExtension` has zero call sites anywhere in `src/` — like Task 16's `confidence_signals_for_claim`, this is a second dormant, tested-in-isolation component the live pipeline never actually calls (the "always routes to Arbiter" behavior is true today simply because nothing branches on H3 at all, not because `DisabledConflictExtension.evaluate()` runs and returns that route). Also: `build_conflict_indicators` needs `claim_evidence_links`, which only exist **after** the Arbiter has already produced a result — so a conflict cannot be known before the Arbiter runs, making "debate instead of calling the Arbiter" structurally impossible. H3 can only be a **post-Arbiter refinement** of one already-produced conclusion claim, not a pre-Arbiter branch. This is a correction to the original brief's framing above, not a scope change.
  - **Files (as actually touched):**
    - Created: `src/hoya_agent/reasoning/conditional_debate.py` — `ConditionalDebateExtension`, a second `ConflictExtension` alongside `DisabledConflictExtension` (that file is untouched). `evaluate()` matches `DisabledConflictExtension.evaluate()`'s exact signature and never calls the LLM. The actual debate is a separate `run_debate(claim, indicator, ledger, deadline) -> DebateOutcome` method — `ConflictExtensionResult` has no field for a revised claim, and adding one would break interface parity with the disabled implementation.
    - Created: `prompts/bull-v1.md`, `prompts/bear-v1.md`, `prompts/judge-v1.md`; registered as `debate_bull`/`debate_bear`/`debate_judge` in `reasoning/prompt_library.py::PROMPT_FILES` (same pattern as Task 16's `semantic_grounding` registration, including the same shared-contract phrases every prompt must state).
    - Created: `tests/unit/reasoning/test_conditional_debate.py` — 10 tests with a fake `LLMClient`: `evaluate()` decision matrix (flag off / no conflict / both / neither — all with an LLM that raises if ever called, proving zero calls), a full successful debate, a non-conclusion claim skipped without a call, evidence missing from the ledger skipped without a call, Bull/Judge failure aborting the whole debate, and the Judge schema itself rejecting `confidence="high"`.
    - **Not modified:** `src/hoya_agent/composition.py`, `orchestration/pipeline.py` — see below.
  - [x] Wrote failing-then-passing tests proving the extension only activates on a real `ConflictIndicator`; with no material conflict (or the flag off), behavior is identical to `DisabledConflictExtension` — proved directly, since `evaluate()` makes zero LLM calls either way, not just asserted.
  - [x] Implemented exactly one Bull round, one Bear round, one Judge call, each citing only ledger Evidence IDs. **Deviation from the brief, deliberate:** the Judge's output schema (`DebateVerdict`) is narrower than reusing `reasoning/arbiter_output.py::ArbiterClaim` wholesale — only `text`/`confidence`/`limitations`/`confidence_rationale` are model-writable; `claim_id`/`claim_type`/`assets`/`time_range`/`based_on_claim_ids` are carried over from the original claim untouched. Reusing `ArbiterClaim` verbatim would have let the model rewrite fields (asset, time range, dependencies) H3 must never touch — the module docstring explains this in full.
  - [x] Any failure at any of the three steps (LLM error, missing evidence, non-conclusion claim) returns `DebateOutcome(None, ...)`; the caller is required to keep the original claim. No stage deadline or token budget was added or extended — `run_debate()` takes the caller's `deadline` as a parameter rather than computing its own.
  - [x] `enable_conditional_debate` was already an opt-in field on `AnalysisRequest` (Task 1a); nothing changed there. `evaluate()`'s flag-off and no-conflict paths are byte-identical in shape to `DisabledConflictExtension`'s.
  - [ ] **Not done:** wiring `ConditionalDebateExtension` into `orchestration/pipeline.py`/`composition.py`, and the corresponding "H3 未實作" → "opt-in, off by default" label updates (correctly left undone, since the task's own instruction is not to change those labels before live acceptance is met — it is not met). See `docs/Implementation-Plan.md` §9 Task 17 for the exact insertion point this would need (after `finalize_analysis()` computes conflict indicators, gated on `context.enable_conditional_debate`) and why it was not attempted without live Bedrock access to verify it — the same reasoning as Task 16's deferred wiring, compounded here by the pipeline's own already-documented Arbiter reliability issue (§8 item 11), which additional LLM calls should not be layered onto blind.
  - [x] Ran `python -m pytest tests/unit/reasoning/test_conditional_debate.py tests/unit/reasoning/test_prompt_library.py -q` (38 passed) then the full non-live suite (1357 passed, up from 1347) and `ruff check .` (All checks passed, after fixing two unused imports the linter caught).
  - **Acceptance:** **Engine met and independently verified with fakes; live-pipeline integration explicitly deferred**, consistent with Task 16. With no conflict or the flag off, `evaluate()` output is provably identical to `DisabledConflictExtension` (zero LLM calls in either case, same status/route shape); a real conflict plus the flag produces `route=DEBATE_ROUTE`, and `run_debate()` on a real conflict produces a valid revised `Claim` that discloses both sides in its `limitations`. No unbounded loop, no new tool, no new provider — only Bedrock via the same `LLMClient` port every other reasoning component already uses.
