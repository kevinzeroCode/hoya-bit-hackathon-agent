...
| UI（S4） | 🔴 未開始 |
| **真實 Bedrock 呼叫（S0）** | 🟡 **已成功呼叫過，但 S0 未關閉** —— 見下方兩個缺口 |
...
> **但 S0 仍關不掉，剩兩個獨立的缺口：**
>
> 1. **AWS 帳號未提交 Anthropic use case details 表單。** 目前每次 Anthropic 模型呼叫都回
>    `ResourceNotFoundException: Model use case details have not been submitted for this account`
>    （連續 4 次實測皆同）。Bedrock 本身可達——`list_foundation_models` 列得到 112 個模型、
>    Haiku 4.5 在清單內，標準 AWS profile 憑證也有效。**純粹卡在主控台這一步。**
>    這不擋離線的 Bronze／Silver，但 Gold 與現場 demo 全靠它。
> 2. **S0 退出條件 #1 要的是一次真實的 *Converse 結構化輸出* 呼叫。** P2 那次走
>    `invoke_model`；`adapters/bedrock.py` 走 `converse` + forced `toolConfig`，不是同一條路。
>    已證明的是憑證／region／模型存取權，未證明的是強制工具呼叫的結構化輸出。
>
