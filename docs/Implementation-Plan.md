...
| **17** H3 條件式辯論 | 真正實作 Bull/Bear/Judge，`enable_conditional_debate` opt-in | 🟡 引擎完成，未接進 live pipeline（2026-08-03） | 動工前先發現：`ConflictExtension` 跟 Task 16 的 `confidence_signals_for_claim` 一樣，全倉庫零呼叫點——「永遠走 Arbiter」是因為 pipeline 根本沒有 H3 分支，不是 `DisabledConflictExtension.evaluate()` 真的被呼叫。另外 `build_conflict_indicators` 需要 `claim_evidence_links`，那是 Arbiter **產出之後**才有的東西，所以 H3 結構上不可能是「辯論或走 Arbiter」的二選一分支，只能是 Arbiter 產出後的單一結論精修——已修正原規劃的描述。新增 `reasoning/conditional_debate.py`（`ConditionalDebateExtension`，`evaluate()` 不呼叫 LLM，實際辯論在獨立的 `run_debate()`）＋ 三份 prompt，10 個 fake-LLM 測試全過。**未接線**：原因與確切接線點見上方「設計發現」一欄同款，理由跟 Task 16 一樣（沒有 live 憑證），這裡還多疊加一層——pipeline 自己的 Arbiter 可靠度問題（§8 項目 11）還沒解，不該在看不到效果的情況下疊加更多 LLM 呼叫 |
...
| **20** PDF/HTML 匯出與額外視覺化 | 在 PR #29 既有的自包含 HTML report 基礎上加 | ✅ 已完成（2026-08-03，兩半都做了） | HTML 早就有了。PDF 踩到兩個真的坑：① 既有畫面版 CSS 用 `var(--x)` 等現代語法，直接餵給 `xhtml2pdf` 直接炸掉，改成從 `final_report.md` 的純文字轉出一份極簡、xhtml2pdf 安全的 HTML；② 繁體中文完全不顯示（`xhtml2pdf` 內建字型沒有 CJK 字形），註冊 Adobe 標準 CJK CID 字型 `MSung-Light` 後解決——用 `pypdf` 把 PDF 文字層抓出來逐字比對驗證，因為這個 session 自己用來看 PDF 的工具本身也沒有 CJK 字型可用，畫面驗證不了。額外視覺化：`html_renderer.py` 新增 `_trust_radar_svg()`，每個結論的 Trust Scorecard 各畫一張五軸雷達圖（純字串拼 SVG，`math` 算三角座標，零新依賴），數值全部來自既有的 0-3 pip 序數，沒有結論 scorecard 時照舊退回純文字摘要，不捏造圖表 |
| **21** S3 鏡像／CloudWatch／ECS | Platinum 基礎設施擴充 | 🟡 應用程式碼完成，只做地端驗證（2026-08-03） | 這次比賽用的 Workshop Studio AWS 帳號本來就是臨時的、活動結束會收回（`docs/deployment.md` 早就記過），這個 session 確認拿不到 live 存取，使用者也明確指示「先做成之後能部署、但這次只需要地端驗得過」。`adapters/s3_mirror.py`／`adapters/cloudwatch_metrics.py` 都沿用 `bedrock.py` 的 client 注入模式，fake client 測試、不需真的 AWS。ECS 評估過但沒採用（單台 EC2 已經滿足唯一真正需要的部署保證），寫了 `deploy/ecs/task-definition.json` 備用，只驗過 JSON 語法。三者都**沒接進** `application.py`，也沒有任何東西真的 apply 到 AWS |
