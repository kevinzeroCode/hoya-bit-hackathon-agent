...
| 1b | _this commit_ | Codex（依使用者指示代為執行，非 Kiro session） | Python 3.12 `compileall` → passed；Pydantic 2.13 runtime schema、official clock freeze、secret snapshot、static allowlist smoke checks → passed；完整 `pytest` / `ruff` 待具備 dev dependencies 的環境重跑 | 實作 `config.py`、`clock.py`、`ports.py`、七個 plumbing models、共用 test bootstrap 與 reusable fakes。沙箱無法連 PyPI，離線 cache 也沒有 pytest/ruff，因此不虛構 Kiro 證據或全綠結果。 |
| 1b (merged to main) | `f7536fb` | Multiple PRs merged to main | `python -m pytest tests/unit tests/contract tests/integration -q` → 501+ passed; `ruff check .` → passed | `config.py`, `clock.py`, `ports.py`, `tests/fakes.py`, `tests/conftest.py` landed on main. Runtime seams complete: typed Settings, injectable Clock with official cutoff freeze, Protocol interfaces for all adapters, shared test doubles (FixedClock, FakeLLM, FakeSourceAdapter), and pytest conftest with session-scoped fixtures. |
| S2 (fixture vertical slice) | `f7536fb` | Multiple PRs merged to main | `python -m pytest tests/integration/test_vertical_slice.py -q` → passed; four artifacts produced offline | `application.py`, `reporting/renderer.py`, `reporting/artifacts.py`, `_provisional_seams.py`, `tests/fixtures/vertical_slice/`, `tests/integration/`. S2 complete: deterministic 11-section zh-Hant renderer, atomic artifact writes (tmp+fsync+replace), provisional runtime types bridging to Task 1b seams, offline integration test producing all 4 fixed artifacts from fixture data. |
| S5 (market/evidence integration) | `f7536fb` | Multiple PRs merged to main | `python -m pytest tests/unit tests/contract tests/integration -q` → 501+ passed | Full `data/` layer integrated: `indicators.py`, `market_worker.py`, `market_series.py`, `regime.py`, `price_analysis.py`. Adapters landed: `organizer_csv.py`, `binance.py`, `cryptopanic.py`, `rss.py`, `alternative_me.py`, `port_adapters.py`. Evidence processor wired to produce high-reliability market EvidenceItems from OHLCV bars. |
| Pipeline (CSV-only) | `f7536fb` | Multiple PRs merged to main | BTC offline run produces 4 artifacts with 4 high-reliability market Evidence Items | `orchestration/pipeline.py` with `OrganizerCsvPipeline` — implements `AnalysisPipeline` seam over organizer CSV. Coin-agnostic (symbol is parameter). Includes `to_contract_ledger()` bridge from frozen `evidence/types.py` dataclasses to `models.py` contracts. Honestly `degraded` without Arbiter, renders insufficient-data report over real evidence. |
| AGENTS.md | `f7536fb` | Kiro codebase summary | N/A — documentation only | Generated codebase documentation: directory map, pipeline architecture, key patterns, frozen paths, deadlines, configuration, and testing instructions. Reflects 44 .py files in `src/hoya_agent/`. |
| S8 Silver live acceptance | PR #26 | Codex implementation + user-operated credentialed run | `python -m pytest tests/live/test_live_silver_pipeline.py -m live -vv -s` → 1 passed in 50.15s; non-live 1143 passed / 3 skipped; Ruff clean | Non-Kiro attribution: integrated Organizer/Binance, CoinDesk RSS, Bedrock extraction/Arbiter and four artifacts in one `ApplicationService` run; no credentials recorded. |
